...
// Polygon related functions
  ...
  // if so return the length of (p1->intersection point) in unit of the length of (p1->p2)
  inline double line_intersect(Point p1, Point p2, Point p3, Point p4, bool& parallel)
  {
    double x1 = p1.x;
    double x2 = p2.x;
    double x3 = p3.x;
  ...
  bool line_segment_intersect(Point p1, Point p2, Point p3, Point p4, Point& intersection){
    ...
    if (parallel || u < 0 || u > 1) return false;
  ...
  void Polygon::add(Point p)
  {
    ...
    for (int i = 1; i < np-1; ++i) { // check that new segment doesn't interset an old one
      Point ignore(0,0);
